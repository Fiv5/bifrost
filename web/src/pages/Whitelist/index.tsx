...
const accessModeOptions: { value: AccessMode; label: string; description: string }[] = [
  { value: 'allow_all', label: 'Allow All', description: 'Allow all connections (no restriction)' },
  { value: 'local_only', label: 'Local Only', description: 'Only allow localhost connections (127.0.0.1, ::1)' },
  { value: 'whitelist', label: 'Whitelist', description: 'Only allow whitelisted IPs/CIDRs' },
  { value: 'interactive', label: 'Interactive', description: 'Prompt for unknown IPs (not implemented)' },
];

const getModeColor = (mode: AccessMode) => {
  switch (mode) {
    case 'allow_all': return 'red';
    case 'local_only': return 'blue';
...
const getModeIcon = (mode: AccessMode) => {
  switch (mode) {
    case 'allow_all': return <GlobalOutlined />;
    case 'local_only': return <LaptopOutlined />;
    case 'whitelist': return <SafetyOutlined />;
    case 'interactive': return <ClockCircleOutlined />;
    default: return null;
  }
};
...
export default function Whitelist() {
  ...
  return (
    <div style={{ padding: 16 }}>
        ...
        <Col>
          <Space>
            <Tag color={getModeColor(status.mode)} icon={getModeIcon(status.mode)}>
              {accessModeOptions.find(o => o.value === status.mode)?.label || status.mode}
            </Tag>
      ...
      <Row gutter={[16, 16]}>
        <Col xs={24} lg={12}>
          <Card title="Access Settings" size="small">
            <Row gutter={[16, 16]}>
              <Col span={24}>
                <Space direction="vertical" style={{ width: '100%' }}>
                  ...
                  <Text type="secondary" style={{ fontSize: 12 }}>
                    {accessModeOptions.find(o => o.value === status.mode)?.description}
                  </Text>
                </Space>
              </Col>
              <Col span={24}>
                ...
                <br />
                <Text type="secondary" style={{ fontSize: 12 }}>
                  When enabled, private network IPs (192.168.x.x, 10.x.x.x, 172.16-31.x.x) are allowed
                </Text>
              </Col>
            </Row>
          </Card>
        ...
        <Col xs={24} lg={12}>
          <Card
            title={`Permanent Whitelist (${status.whitelist.length})`}
            size="small"
            extra={
              <Space.Compact>
                <Input
        ...
        <Col xs={24}>
          <Card
            title={
              <Space>
                <ClockCircleOutlined />
                <span>Temporary Whitelist ({status.temporary_whitelist.length})</span>
              </Space>
            }
            size="small"
            extra={
          ...
          >
            <Text type="secondary" style={{ display: 'block', marginBottom: 8 }}>
              Temporary entries are stored in memory and will be lost when the server restarts.
            </Text>
            <Table
              columns={tempColumns}
