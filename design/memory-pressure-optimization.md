...
## 现象与关键路径
- 请求侧：HTTP/HTTPS 请求体在需要处理时被完整收集到内存，且存在多次拷贝与字符串化。
- 响应侧：非处理分支已支持流式 tee，但 SSE 事件缓冲无上限增长风险。
- 连接监控：连接关闭后未周期清理，历史连接数累积。
- BodyStore：过期文件清理依赖显式调用，无定时清理任务。

## 核心问题
1. 请求体在处理分支与跳过分支都全量读取，导致大请求峰值内存放大。
2. SSE 事件缓冲在无边界或长连接场景可能持续增长。
3. 连接监控与 body 存储缺少周期性清理，造成长期内存水位抬升。
4. 脚本执行对 body 字符串化引入不必要的拷贝。
...
- 请求与响应均使用较大体积（可配置），统计：
  - RSS 实时变化（通过管理接口获取内存指标）
    ...
    - 使用方式示例：
      - curl -s 'http://127.0.0.1:9900/_bifrost/api/metrics' | jq '{ts:.timestamp, rss_bytes:.memory_used, rss_mb:(.memory_used/1024/1024)}'
  - 成功率与耗时
